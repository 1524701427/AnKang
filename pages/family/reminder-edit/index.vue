...
<view class="page">
  <view class="form card">
    ...
    <picker :range="typeLabels" @change="e=>typeIndex=e.detail.value"><view class="input">{{typeLabels[typeIndex]}}</view></picker>

    <text class="label">提醒名称</text>
    <input class="input" v-model="form.title" placeholder="例如：吃早饭、服用药物"/>

    <text class="label">时间</text>
    <picker mode="time" :value="form.scheduleTime" @change="e=>form.scheduleTime=e.detail.value"><view class="input">{{form.scheduleTime}}</view></picker>

    <text class="label">剂量 / 内容（可选）</text>
    <input class="input" v-model="form.dosage" placeholder="例如：1片"/>

    <text class="label">未确认多久通知家人</text>
    <picker :range="notifyLabels" @change="e=>notifyIndex=e.detail.value"><view class="input">{{notifyLabels[notifyIndex]}}</view></picker>

    <view class="switch-row"><text>启用提醒</text><switch :checked="form.enabled" color="#2fa99a" @change="e=>form.enabled=e.detail.value"/></view>
...
export default{
  data(){return{
    elderId:'',loading:false,typeIndex:0,notifyIndex:2,
