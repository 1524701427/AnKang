...
<view class="page">
  ...
  <view v-else-if="elders.length">
    <view v-for="m in elders" :key="m.id" class="card">
      ...
      <text class="status" :class="{bound:m.bindStatus==='BOUND'}">{{m.bindStatus==='BOUND'?'已绑定':'待邀请'}}</text>
    </view>
  </view>
