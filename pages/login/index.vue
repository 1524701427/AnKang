...
export default{
  ...
  methods:{
    login(){if(!/^1\d{10}$/.test(this.phone)||this.password.length<6){return uni.showToast({title:'请输入正确账号信息',icon:'none'})}uni.showToast({title:'登录成功',icon:'success'});setTimeout(()=>uni.reLaunch({url:'/pages/family/home/index'}),500)},
